pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE_NAME="aws09-spring-petclinic"
            ...
            steps {
                ...
                dir("${env.WORKSPACE}") {
                    ...
                    withAWS(region:"${REGION}", credentials:"${AWS_CREDENTIAL_NAME}"){
                      s3Upload(file:"deploy.zip", bucket:"aws09-codedeploy-bucket")
                    } 
                    sh 'rm -rf ./deploy.zip'                 
                }        
            }
            ...
            steps {
               echo "create Codedeploy group"   
                sh '''
                    ...
                    --application-name aws09-code-deploy \
                    --auto-scaling-groups aws09-asg \
                    --deployment-group-name aws09-code-deploy-${BUILD_NUMBER} \
                    --deployment-config-name CodeDeployDefault.OneAtATime \
                    --service-role-arn arn:aws:iam::257307634175:role/aws09-codedeploy-service-role
                    '''
                echo "Codedeploy Workload"   
                sh '''
                    aws deploy create-deployment --application-name aws09-code-deploy \
                    --deployment-config-name CodeDeployDefault.OneAtATime \
                    --deployment-group-name aws09-code-deploy-${BUILD_NUMBER} \
                    --s3-location bucket=aws09-codedeploy-bucket,bundleType=zip,key=deploy.zip
                    '''
                    sleep(10) // sleep 10s
            }
         }    
